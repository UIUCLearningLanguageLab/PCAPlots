    def make_principal_comps_termgroup_heatmap_fig(tokengroup_tokens_dict,
        ...
        pca_model = sklearnPCA(n_components=config.Fig.NUM_PCS)
        ...
        # fig
        fig, ax_heatmap = plt.subplots(figsize=(config.Fig.MAX_FIG_WIDTH, 0.2 * num_tokengroups),
                                       dpi=config.Fig.DPI)
    ...
    def make_principal_comps_item_heatmap_fig(pca_item_list, label_expl_var=True):
        ...
        # fig
        width = min(config.Fig.MAX_FIG_WIDTH, config.Fig.NUM_PCS * 1.5 - 1)
    ...
    def make_principal_comps_table_fig(pc_id):
        ...
        # fig
        fig, ax = plt.subplots(figsize=(config.Fig.MAX_FIG_WIDTH, 0.25 * max_rows), dpi=config.Fig.DPI)
